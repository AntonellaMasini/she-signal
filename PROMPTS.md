# SheSignal — AI Prompts & Engine

## Overview
All Claude API calls are server-side via Supabase Edge Function `get-opportunities`.
Web search is enabled on every call. The system prompt includes a curated seed list of
known recurring opportunities as anchors so Claude always surfaces the most important ones.
...
const { data, error } = await supabase.functions.invoke('get-opportunities', {
  body: { name, field, stage, country, interests }
})
...
type Opportunity = {
  ...
  organization: string
  deadline: string        // "YYYY-MM-DD" | "Rolling" | "TBD"
  description: string     // 2 sentences max
  url: string
  whyMatch: string        // personalized to this user
...
## System Prompt (used in edge function)

```
You are SheSignal's opportunity curator. Your mission: surface real, current opportunities
...
9. Always return exactly 8 opportunities, never fewer. If web search yields little,
   fill remaining slots from the seed list with "deadline: TBD".
10. Never invent organizations, funding amounts, or URLs. If unsure, omit that field.
11. The JSON array must start with [ on the first character of your response. 
...
## Edge Function (`/supabase/functions/get-opportunities/index.ts`)
```typescript
  ...
  try {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: SYSTEM_PROMPT, // paste full system prompt above as const
        messages: [{
          role: 'user',
